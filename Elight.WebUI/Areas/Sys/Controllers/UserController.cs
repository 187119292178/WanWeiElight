...
namespace Elight.WebUI.Areas.Sys.Controllers
{
    ...
    public class UserController : BaseController
    {
        ...
        /// <param name="parms"></param>
        /// <returns></returns>
        [HttpPost, Route("app/system/user/resetPassword")]
        public ActionResult AppResetPassword([FromBody] UserDeleteParms parms)
        {
            //过滤系统管理员
            if (userLogic.ContainsUser("admin", parms.userIdList.ToArray()))
            {
                return AppError("不能重置系统管理员密码");
            }
            if (parms.userIdList.Contains(parms.currentUserId))
            {
                return AppError("不能重置自己密码，请从账号管理中修改密码");
            }
            //多用户删除。
            SysUserLogOn sysUserLogOn = userLogOnLogic.GetByAccount(parms.userIdList[0]);
            sysUserLogOn.Password = "123456".MD5Encrypt().DESEncrypt(sysUserLogOn.SecretKey).MD5Encrypt();
            int row = userLogOnLogic.UpdatePassword(sysUserLogOn);
            Logger.OperateInfo($"用户{parms.operateUser}重置了用户{parms.userIdList[0]}的密码");
            return row > 0 ? AppSuccess("该用户密码已重置为123456") : AppError("重置失败");
        }
    }
}
