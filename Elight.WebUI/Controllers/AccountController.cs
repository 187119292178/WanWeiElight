...
namespace Elight.WebUI.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public AccountController(ICaptcha captcha)
        {
            _captcha = captcha;
        ...
        {
            string uuid = UUID.StrSnowId;
            CaptchaData info = _captcha.Generate(uuid);
            HttpContext.Session.SetString(Keys.SESSION_KEY_VCODE, uuid);
        ...
        public ActionResult Login(string userName, string password, string verifyCode)
        {
            ...
            if (!_captcha.Validate(HttpContext.Session.GetString(Keys.SESSION_KEY_VCODE), verifyCode))
            {
        ...
        /// </summary>
        /// <returns></returns> 
        [HttpPost, Route("app/account/uploadImage")]
