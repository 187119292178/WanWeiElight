...
{
    [PageCode("wip-packing")]
    public partial class PCBAPackingPage : MyPage
    {
        ...
        private void PackingPage_Load(object sender, EventArgs e)
        {
            ...
            txtOperCode.Text = "Packing-整机包装工站";
        ...
        private bool CheckScan(string sInput, ref string msg)
        {
            ...
            //add 必须为成品的料号
            if (!"成品".Contains(SNResInfo.MatType))
            {
                msg = $"条码[{sInput}]对应料号[{SNResInfo.ItemCode}]的物料类型是[{SNResInfo.MatType}],必须是成品";
                PlaySound(false);
                return false;
            }
        ...
        public bool PrintCartonLabel()
        {
            ...
            //获取物料编码型号规格
            BasItem item = itemLogic.Get(SNResInfo.ItemCode);
            if ((item == null))
            {
                sMsg = $"料号[{SNResInfo.ItemCode}]在系统中不存在";
                return bFlag;
            }

            string LabelPath = AppDomain.CurrentDomain.BaseDirectory + @"Lab\" + basTemplate.RemoteName;

            bFlag = PrintHelper.PrintCartonLabel(labApp, LabelPath, 1, TxtSW.Text.Trim(), ScanList, item.ItemDesc, ref sMsg);
        ...
        {
            RePrintPacking form = new RePrintPacking();
